...
The release includes the release artifact `install-botctrl` which can be used to install the `botctrl` management system. See the [Install botctrl](#install-botctrl) section below for installation instructions. See the [repository README](https://github.com/missyrestless/Bots) for additional info and example `botctrl` command invocations.
...
Alternatively, download the `install-botctrl` release artifact and execute it. The `install-botctrl` script will clone the repository and install the system:
...
The `botctrl` command is installed in `/usr/local/bin` along with some utility scripts for use with `cron` or other management systems. These utility scripts will need to be modified to suit your specific needs, configuration and bot names. You can modify the scripts in `BotControl/bin/` and re-run `./install-botctrl`.
