...
state sense {
    ...
    http_request(key id, string method, string body) {
        ...
        list restart = llParseString2List(
            wasURLUnescape(
                wasKeyValueGet(
                    "message",
                    body
        ...
        RESTART_DELAY = llList2Integer(restart, 1);
        string timeFrame = llList2String(restart, 2);
        ...
        llInstantMessage(
            (key)CORRADE, wasKeyValueEncode(
                [
                    ...
                    "password", wasURLEscape(PASSWORD),
                    "entity", "region",
                    "position", wasURLEscape("<128, 128, 50>"),
...
state confront {
    state_entry() {
        ...
        llSetTimerEvent(RESTART_DELAY + 60);
    }
    timer() {
