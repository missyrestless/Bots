//////////////////////////////////////////////////////
//  Copyright (C) 2014-2015 Wizardry and Steamworks //
...
default {
    state_entry() {
        if(llGetInventoryType("Corrade-Configuration") != INVENTORY_NOTECARD) {
            llOwnerSay("Sorry, could not find a Corrade-Configuration inventory notecard.");
            return;
        }
        // DEBUG
    ...
    dataserver(key id, string data) {
        if(data == EOF) {
            ...
            if(WAIT == 0) {
                llOwnerSay("Error in Corrade-Configuration notecard: wait");
                return;
            }
            // DEBUG
            llOwnerSay("Read Corrade-Configuration notecard...");
            state read;
        }
